...
def _filter_to_still_open_prs(pr_numbers: List[int]) -> List[int]:
    ...
    # Use concurrency to parallelize network I/O (`gh pr view`) to check multiple
    # PR states efficiently while ensuring the original order via `executor.map`.
    with concurrent.futures.ThreadPoolExecutor(max_workers=min(32, len(pr_numbers) + 4)) as executor:
