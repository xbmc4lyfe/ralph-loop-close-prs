...
def _check_pr_open_wrapper(pr: int) -> Tuple[int, Optional[bool], Optional[Exception]]:
    try:
...
def _filter_to_still_open_prs(pr_numbers: List[int]) -> List[int]:
    ...
    # Check open state concurrently since _pr_is_still_open invokes 'gh' subprocesses
    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
        for pr, still_open, exc in executor.map(_check_pr_open_wrapper, pr_numbers):
            if exc is not None:
